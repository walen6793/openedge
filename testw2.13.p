...
FOR EACH S_Artikel WHERE S_Artikel
  where S_Artikel.Firma    = {firma/sartikel.fir '100'}
    ...
    FIND S_ArtikelSpr
      where S_ArtikelSpr.Firma    = {firma/sartikel.fir '100'}
                ...
                do:
                   FIND S_Kunde
                     where S_Kunde.Firma  = {firma/s_kunde.fir '100'}
                       ...
                       no-lock no-error.
                    ASSIGN
                      tt-Result.tt-KundeID = bS_ArtKunde.Kunde
        ...
        FOR EACH bS_ArtLief
          where bS_ArtLief.Firma = {firma/sartlief.fir '100'}
            and  bS_ArtLief.Artikel = S_Artikel.Artikel
            no-lock:
              tt_Result.tt-LiefCount = tt_Result.tt-LiefCount + 1.
              if tt-Result.tt-LiefCount = 1 then
                do:
                  FIND S_Lieferant
                    where S_Lieferant.Firma = {firma/s_liferant.fir '100'}
                    AND S_Lieferant.Lieferant = bS_ArtLief.Lieferant
                    no-lock no-error.
                    ASSIGN
                      tt-Result.tt-LiefID = bS_ArtLief.Lieferant
...
FOR EACH tt_Result
  WHERE tt_Result.tt-KundeCount = 1
    and tt_Result.tt-LiefCount = 1
    no-lock:
